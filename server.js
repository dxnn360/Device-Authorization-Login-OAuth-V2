...
app.get('/halo', verifyToken, (req, res) => {
	console.log('======== Requesting userInfo claims using valid token');
		deviceFlow.userInfo(req.session.token)
		.then((response) => {
			res.render('halo', {message: 'successfully authenticated', userInfo: response.response});
		}).catch((err) => {
			res.send(err);
		});
});
...
function pollToken(device_code, sessionID ) {
	...
	.then(async(response) => {
		...
		try {
			...
			console.log('========= Error setting storage for session');
			return error;
		}
	})
	.catch((err) => {
